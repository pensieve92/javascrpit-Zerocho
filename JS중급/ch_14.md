...
// tick과 2초에 관한 정보는 호출스택이 가지고 있지 않는다. >> 이때 나오는 개념이 [이벤트루프] 백그라운드 

// 백그라운드는 노드 내부 엔진 동작이라고 생각하세요~
tick, 2초마다 >> 호축스택에 넣어줌

// 태스크 큐
백그라운드에 있는 함수를 담아두는곳
// 이벤트 루프
태스크 큐에 있는 함수의 순서를 정해준다.
호출스택이 비어 있으면 태스크 큐에있는 함수를 호출스택으로 올려준다.
```
...
블록 스코프 {} function, if, for 문 안에서만 살아갈수 있다.
```javascript
if(true){
    const a = 1;
}
